...
export function ProjectsSection() {
  ...
  const carousel = useScrollReveal(0.1);
  ...
  return (
    <section id="work" className="relative pt-8 sm:pt-12 pb-16 sm:pb-24">
      ...
      <div className="mx-auto max-w-6xl px-6">
        <div ref={heading.ref} className="mb-16 sm:mb-24">
          <div
          ...
          <div className="flex items-end justify-between gap-8">
            ...
            >
              <button
                ...
                className="flex h-12 w-12 items-center justify-center rounded-full border border-foreground/[0.1] text-foreground/40 transition-all hover:border-foreground/25 hover:text-foreground/70 disabled:opacity-20 disabled:pointer-events-none cursor-pointer"
              ...
              <button
                ...
                className="flex h-12 w-12 items-center justify-center rounded-full border border-foreground/[0.1] text-foreground/40 transition-all hover:border-foreground/25 hover:text-foreground/70 cursor-pointer"
      ...
      <div
        ...
        className={`relative transition-all duration-700 ease-out ${
          carousel.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
        }`}
          ...
          {loopedProjects.map((project, i) => {
            ...
            return (
              <div
                key={`${project.title}-${i}`}
                data-project-card
                className={`group relative flex-shrink-0 w-[85vw] sm:w-[520px] snap-start transition-all duration-700 ease-out ${
                  carousel.visible
                    ? "opacity-100 translate-y-0"
                ...
                style={{
                  pointerEvents:
                    dragState.current.hasMoved && isDragging ? "none" : "auto",
                }}
              >
                <div
                  className={`relative overflow-hidden rounded-2xl border transition-all duration-500 project-card-hover ${
                    isActive
                      ? "border-foreground/[0.1] scale-100"
                      : "border-foreground/[0.04] scale-[0.97] opacity-60"
                  }`}
                >
                  {/* Image area */}
                  <div className="relative aspect-[4/3] w-full overflow-hidden bg-foreground/[0.03]">
                    <div className="absolute inset-0 bg-gradient-to-br from-foreground/[0.02] via-transparent to-foreground/[0.04]" />

                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="font-serif italic text-[clamp(7rem,18vw,12rem)] leading-none text-foreground/[0.04] select-none transition-all duration-700 group-hover:text-foreground/[0.07] group-hover:scale-105">
                        {num}
                      </span>
                    </div>

                    {/* Corner accents */}
                    <div className="absolute top-0 left-0 w-12 h-px bg-foreground/[0.08] group-hover:w-20 transition-all duration-500" />
                    <div className="absolute top-0 left-0 w-px h-12 bg-foreground/[0.08] group-hover:h-20 transition-all duration-500" />
                    <div className="absolute bottom-0 right-0 w-12 h-px bg-foreground/[0.08] group-hover:w-20 transition-all duration-500" />
                    <div className="absolute bottom-0 right-0 w-px h-12 bg-foreground/[0.08] group-hover:h-20 transition-all duration-500" />
                  ...
                  <div className="relative p-7 sm:p-8 -mt-6">
                    <div className="flex items-center gap-3 mb-5">
                      ...
                      <div className="h-px flex-1 bg-foreground/[0.06] group-hover:bg-foreground/[0.12] transition-colors duration-500" />
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {project.github && (
                        <a
                          ...
                          className="group/btn inline-flex h-9 items-center gap-2 rounded-full border border-foreground/[0.1] px-5 text-[0.7rem] font-medium uppercase tracking-[0.12em] text-foreground/50 transition-all hover:border-foreground/25 hover:text-foreground/80"
                        >
