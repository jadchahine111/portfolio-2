...
export function AboutSection() {
  ...
  const statsBlock = useScrollReveal(0.2);
  const philosophy = useScrollReveal(0.1);
  const closing = useScrollReveal(0.2);
  ...
  return (
    <section id="about" className="relative pt-16 sm:pt-24 pb-16 sm:pb-24 px-6 overflow-hidden">
      <div className="mx-auto max-w-6xl">
        ...
        {/* Stats row */}
        <div ref={statsBlock.ref} className="mb-24 sm:mb-32">
          <div
            className={`h-px w-full bg-foreground/[0.08] mb-10 transition-all duration-1000 ease-out origin-left ${
              statsBlock.visible ? "scale-x-100" : "scale-x-0"
            }`}
          />
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 sm:gap-8">
            {stats.map((stat, i) => (
              ...
              >
                <span className="block font-serif text-[clamp(2.5rem,5vw,3.5rem)] italic leading-[1] tracking-tight text-foreground">
                  {stat.value}
                </span>
          ...
          <div
            className={`h-px w-full bg-foreground/[0.08] mt-10 transition-all duration-1000 ease-out origin-right ${
              statsBlock.visible ? "scale-x-100" : "scale-x-0"
            }`}
